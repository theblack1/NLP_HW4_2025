...
def exp_kshot(tokenizer, model, inputs, k):
    ...
    for i, sentence in tqdm(enumerate(inputs), desc="Prompting", total=len(inputs)):
...
if __name__ == "__main__":
    ...
    print("Starting prompting.py process...")
    main()
    end_time = time.time()
    elapsed = end_time - start_time
    elapsed_hours = elapsed / 3600
    print(f"prompting.py Total execution time: {elapsed:.2f} seconds ({elapsed_hours:.2f} hours)")